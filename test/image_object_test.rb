...
class ImageObjectAdminTest < Minitest::Test
  ENTITY = "ImageObject"
  BASE = "/image-objects"
  ...
  def test_stored_dangerous_urls_render_as_inert_text_never_as_links
    js_id = AT.seed_with(stack, ENTITY, { "contentUrl" => "javascript:alert(1)" }, jar)
    ...
    data_id = AT.seed_with(stack, ENTITY, { "contentUrl" => "data:text/html,x" }, jar)
  ...
  def test_stored_http_url_renders_as_a_clickable_link
    item_id = AT.seed_with(stack, ENTITY, { "contentUrl" => "https://example.com/profile" }, jar)
